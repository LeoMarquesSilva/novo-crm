...
- **Fase atual:** Fase 2 — Primitives — **concluída** (dois lotes). Checkbox/RadioGroup não existem como componentes no repo (nada a migrar); Avatar e Label revisados e deixados como estão (já neutros/próximos do V2, risco de mudar não compensa o ganho agora)
