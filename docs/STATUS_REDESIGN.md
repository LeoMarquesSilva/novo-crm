...
| 14/09/2026 | Tokens semânticos shadcn (`--background`, `--primary`, `--border`, `--input`, `--ring`, `--destructive`, paleta de gráficos) foram sobrescritos direto pros valores V2 já na Fase 1, sem alias temporário | Claude (Lead) | Blast radius mapeado por grep antes da troca: consumo direto e contido (badge default, botão link, focus rings dos primitives shadcn, borda base global) — nenhum componente visualmente pesado (Button, Card, sidebar) usa esses tokens diretamente, todos usam classes/cores próprias (`.glass-card`, `--accent-teal`, `--primary-dark`) que só migram na Fase 2/3 |
