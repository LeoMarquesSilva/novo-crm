...
export function ContractDetailShell({ contract, canConfigure }: { contract: ContractDetailViewModel; canConfigure: boolean }) {
  ...
  return <div className="space-y-5">
    ...
    <Tabs defaultValue="overview" className="gap-4">
      ...
      <TabsContent value="closings"><Collection title="Fechamentos" empty="Nenhum fechamento registrado.">{contract.closings.map((closing) => <article key={closing.id} className="rounded-xl border border-zinc-200 p-4"><div className="flex justify-between gap-3"><strong>{closing.competency}</strong><Badge variant="outline">{closing.status.replaceAll("_", " ")}</Badge></div><p className="mt-1 text-sm text-zinc-500">Revisão {closing.revision}</p></article>)}</Collection></TabsContent>
