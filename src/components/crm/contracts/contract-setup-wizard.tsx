...
export function ContractSetupWizard({ contract, canConfigure, returnTarget }: { contract: ContractDetailViewModel; canConfigure: boolean; returnTarget: string }) {
  ...
  const [invalidEditors, setInvalidEditors] = useState<Record<string, boolean>>({});
  const dirty = configuration !== null && stringify(configuration) !== baseline;
  const hasInvalidJson = Object.values(invalidEditors).some(Boolean);
  const setEditorInvalid = (editor: string, invalid: boolean) => setInvalidEditors((current) => current[editor] === invalid ? current : { ...current, [editor]: invalid });
  ...
  return <section className="space-y-4" aria-label="Assistente de configuração do contrato">
    <ol className="grid gap-2 sm:grid-cols-3 xl:grid-cols-6">
      {steps.map((label, index) => <li key={label}><button type="button" disabled={hasInvalidJson && index !== step} onClick={() => setStep(index)} aria-current={step === index ? "step" : undefined} className={`w-full rounded-xl border p-3 text-left text-xs font-medium disabled:cursor-not-allowed disabled:opacity-50 ${step === index ? "border-teal-500 bg-teal-50 text-teal-900" : "border-zinc-200 bg-white text-zinc-600"}`}><span className="mb-1 block text-[10px] uppercase tracking-wide">Etapa {index + 1}</span>{label}</button></li>)}
    </ol>
    ...
    <div className="rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm">
      ...
      {step === 3 ? <JsonEditor editorKey="allocations" label="Rateios por área" value={configuration.version.areaAllocations} disabled={!canConfigure} onValidityChange={setEditorInvalid} onChange={(areaAllocations) => patchVersion({ areaAllocations })} /> : null}
      {step === 4 ? <div className="grid gap-4 xl:grid-cols-2"><JsonEditor editorKey="shares" label="Participações de sócios" value={configuration.version.partnerShares} disabled={!canConfigure} onValidityChange={setEditorInvalid} onChange={(partnerShares) => patchVersion({ partnerShares })} /><JsonEditor editorKey="commissions" label="Comissões" value={configuration.version.commissions} disabled={!canConfigure} onValidityChange={setEditorInvalid} onChange={(commissions) => patchVersion({ commissions })} /></div> : null}
      ...
      <div className="mt-5 flex flex-wrap items-center justify-between gap-2"><Button type="button" variant="outline" onClick={() => setStep((value) => Math.max(0, value - 1))} disabled={step === 0 || hasInvalidJson}><ChevronLeft />Anterior</Button><div className="flex gap-2"><Button type="button" variant="outline" onClick={saveDraft} disabled={!canConfigure || busy || hasInvalidJson}><Save />Salvar rascunho</Button>{step < steps.length - 1 ? <Button type="button" disabled={hasInvalidJson} onClick={() => setStep((value) => Math.min(steps.length - 1, value + 1))}>Próxima<ChevronRight /></Button> : <Badge className="gap-1 bg-teal-100 text-teal-800"><Check className="size-3" />Etapa final</Badge>}</div></div>
...
function JsonEditor<T>({ editorKey, label, value, disabled, onChange, onValidityChange }: { editorKey: string; label: string; value: T; disabled: boolean; onChange: (value: T) => void; onValidityChange: (editor: string, invalid: boolean) => void }) {
  const [text, setText] = useState(() => stringify(value));
  const [error, setError] = useState<string | null>(null);
  return <label className="block text-sm font-medium text-zinc-700"><span>{label}</span><textarea data-contract-json="true" aria-invalid={Boolean(error)} spellCheck={false} className="mt-1 min-h-64 w-full overflow-auto rounded-xl border border-zinc-200 bg-zinc-950 p-3 font-mono text-xs text-zinc-100 focus:border-teal-500 focus:outline-none" value={text} disabled={disabled} onChange={(event) => { const next = event.target.value; setText(next); try { onChange(JSON.parse(next) as T); setError(null); onValidityChange(editorKey, false); } catch { setError("JSON inválido; corrija antes de salvar."); onValidityChange(editorKey, true); } }} />{error ? <span className="mt-1 block text-xs text-rose-700">{error}</span> : null}</label>;
}
