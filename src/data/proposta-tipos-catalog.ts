...
/** Valor persistido em `cp_escopo_detalhe_json` (uma combinação por área selecionada). */
...
/** Investimento por área usa `cp_escopo_detalhe_json.investimento` e o catálogo global. */
const INV_UM_PROCESSO = "";
...
function st(
  ...
  const fromText = extractPlaceholderKeysFromText(escopoTemplate, investimentoTemplate);
  ...
  Cível: [
    {
      ...
      label: "Contencioso",
      subtipos: [
        st("um_processo", "1 processo", ESCOPO_UM_PROCESSO, INV_UM_PROCESSO, [
          PROPOSTA_PLACEHOLDER_RESUMO_PROCESSO,
        ]),
        st("mais_um_processo", "+1 processo", ESCOPO_MAIS_PROCESSOS, INV_MAIS_PROCESSOS),
        st(
          "carteira_processos",
